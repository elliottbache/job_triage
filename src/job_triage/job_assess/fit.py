...
from job_triage.job_assess.salary import estimate_salary
...
class StackFitResult(BaseModel):
...
def evaluate_job_fit(
    ...
    scored_stack_mentions = create_scored_stack_mentions(
        job_post_extraction=job_post_extraction,
        job_post_assessment=job_post_assessment,
    )
    stack_fit_result = compare_my_stack_to_theirs(
        scored_stack_mentions=scored_stack_mentions
    )
    skill_fit_scores = skill_fit_scores_for_assessment(
        job_post_assessment=job_post_assessment,
    ...
    if not validate_seniority_location_salary(
        ...
        return StackFitResult(score=0, skill_fit_scores=skill_fit_scores)
...
def skill_fit_scores_for_assessment(
    *,
    job_post_assessment: JobPostAssessment,
    stack_fit_result: StackFitResult,
) -> dict[str, float]:
...
def create_scored_stack_mentions(
    *,
    job_post_extraction: JobPostExtraction,
    job_post_assessment: JobPostAssessment,
) -> list[ScoredStackMention]:
...
def compare_my_stack_to_theirs(
    *,
    scored_stack_mentions: list[ScoredStackMention],
    ...
    all_skill_groups = group_all_substitute_skills(scored_stack_mentions)
    if not all_skill_groups:
        return StackFitResult(score=100, skill_fit_scores={})
    ...
    my_stack = read_my_stack(my_path)
    not_in_my_stack = set()
    total_fit = 0.0
    skill_fit_scores = {}
...
def group_all_substitute_skills(
    scored_stack_mentions: list[ScoredStackMention],
) -> list[list[ScoredStackMention]]:
    ...
    for scored_stack_mention in scored_stack_mentions:
        ...
        skill_group = group_single_substitute_skill(
...
def group_single_substitute_skill(
    *,
    scored_stack_mention: ScoredStackMention,
    scored_stack_mentions: list[ScoredStackMention],
) -> list[ScoredStackMention]:
    ...
    for substitute in substitutes:
        skill = get_scored_stack_mention(substitute, scored_stack_mentions)
        if skill is None:
...
def get_scored_stack_mention(
    skill: str, scored_stack_mentions: list[ScoredStackMention]
) -> ScoredStackMention | None:
...
def read_my_stack(path: Path) -> dict[str, int]:
...
def calculate_skill_fit(
    *,
    ...
    """
    grade = grade_required_stack(skill)
...
def validate_seniority_location_salary(
